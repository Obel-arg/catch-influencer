...
export class MissingMetricsWorker {
  ...
  private async checkMissingMetrics(): Promise<void> {
    try {
      ...
      console.log(
        `🔍 WARN[MISSING_METRICS_WORKER] Found ${missingPosts.length} posts missing metrics.`,
      );
      ...
      this.errorCount++;
      console.error(
        '❌ [MISSING_METRICS_WORKER] Error checking missing metrics:',
        error,
      );
    }
  }
  ...
  private async findMissingMetricsPosts(): Promise<MissingMetricsResult[]> {
    try {
      ...
      const { data: missingPosts, error } = await supabase.rpc(
        'find_posts_without_metrics',
        {
      ...
      console.error(
        '❌ [MISSING_METRICS_WORKER] Error finding missing metrics posts:',
        error,
      );
      throw error;
    }
  ...
  private async findMissingMetricsPostsAlternative(): Promise<
    MissingMetricsResult[]
  > {
    try {
      ...
      for (const post of missingPosts as InfluencerPost[]) {
        ...
        if (metricsError) {
          console.warn(
            `⚠️ [MISSING_METRICS_WORKER] Error checking metrics for post ${post.id}:`,
            metricsError,
          );
          continue;
        }
      ...
      console.error(
        '❌ [MISSING_METRICS_WORKER] Error in alternative query:',
        error,
      );
      throw error;
    }
  ...
  private async processMissingMetricsPost(
    post: MissingMetricsResult,
  ): Promise<void> {
    try {
      ...
      console.error(
        `❌ [MISSING_METRICS_WORKER] Error processing post ${post.postId}:`,
        error,
      );
  ...
  private async triggerMetricsProcessing(
    post: MissingMetricsResult,
  ): Promise<void> {
    try {
      ...
      console.log(
        `➡️ [MISSING_METRICS_WORKER] Triggering metrics processing for post ${post.postId}`,
      );
      ...
      console.log(
        `✅ [MISSING_METRICS_WORKER] Job queued for post ${post.postId}, job ID: ${data_result}`,
      );
    } catch (error) {
      console.error(
        `❌ [MISSING_METRICS_WORKER] Error triggering metrics processing for post ${post.postId}:`,
        error,
      );
      throw error;
    }
  ...
  private async logMissingMetricsEvent(
    post: MissingMetricsResult,
  ): Promise<void> {
    try {
      const eventData = {
      ...
      const { error } = await supabase.from('worker_logs').insert(eventData);

      if (error) {
        console.warn(
          `⚠️ [MISSING_METRICS_WORKER] Could not log event: ${error.message}`,
        );
      }
    } catch (error) {
...
export async function processMissingMetricsJob(
  job: MissingMetricsJob,
): Promise<void> {
