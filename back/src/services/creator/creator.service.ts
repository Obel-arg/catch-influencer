...
export class CreatorDBService {
  ...
  static async get(
    ...
    try {
      ...
      if (
        error.response?.data?.error === 'AuthFailed' ||
   ...
   */
  static extractPlatformAndId(
    url: string,
  ): { platform: string; platformUserId: string } | null {
    try {
  ...
  static async submitCreatorWithHistory(
    ...
    try {
      ...
      const creatorDbResponse = await this.submitCreators(platform, [
        platformUserId,
      ]);
  ...
  static async searchInfluencers(filters: Record<string, any>) {
    ...
    if (!supportedPlatforms.includes(normalizedPlatform)) {
      console.warn(
        `Búsqueda avanzada para la plataforma '${platform}' aún no implementada.`,
      );
      return {
        items: [],
        count: 0,
    ...
    try {
      ...
      const searchResponse = (await creatorDBClient.post(
        endpoint,
        body,
      )) as any;
      ...
      let processingPlatform =
        normalizedPlatform === 'all' ? 'instagram' : normalizedPlatform;
      ...
      if (hasStrictFilters && sortedResults.length > 0) {
        const validation = this.validateCreatorDBResults(
          sortedResults,
          filters,
        );
        ...
        if (
          ...
          validation.validResults.length < sortedResults.length
        ) {
        ...
        switch (status) {
          case 403:
            if (apiError === 'FilterValueType') {
              errorMessage =
                'Error en el formato de filtros. Los valores numéricos deben ser números, no texto. Verifique minFollowers, maxFollowers, minEngagement y maxEngagement.';
              errorCode = 'FILTER_VALUE_TYPE_ERROR';
              console.error(
                '🔧 [FIX HINT] Los filtros enviados contienen valores string donde se esperan números. Verifique la conversión de tipos en buildInstagramSearchBody.',
              );
            } else {
              errorMessage =
                'Acceso denegado a la API de CreatorDB. Verifique las credenciales.';
              errorCode = 'API_ACCESS_DENIED';
            }
            break;
          case 400:
            errorMessage =
              'Solicitud inválida. Verifique los parámetros de búsqueda.';
            errorCode = 'INVALID_REQUEST';
            break;
          case 429:
            errorMessage =
              'Límite de solicitudes excedido. Intente nuevamente en unos minutos.';
   ...
   */
  private static async processYouTubeSearchResults(
    ...
    const allResults = await Promise.allSettled(
      influencerIds.map(async (id: string, index: number) => {
        ...
        try {
          ...
          const instagramId =
            ytData.instagramId && ytData.instagramId.trim() !== ''
              ? ytData.instagramId
              : null;
          const tiktokId =
            ytData.tiktokId && ytData.tiktokId.trim() !== ''
              ? ytData.tiktokId
              : null;
          const youtubeId =
            ytData.youtubeId && ytData.youtubeId.trim() !== ''
              ? ytData.youtubeId
              : null;
          ...
          const youtubeEngagement =
            ytData.engageRate1Y || ytData.engageRate || 0;
          ...
          if (
            ...
            bestAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(
              (ytData.youtubeName || 'U').charAt(0),
          ...
          const platformInfo: any = {
            youtube: youtubeId
              ? {
                  ...
                  engageRate1Y: youtubeEngagement,
                  avatar: bestAvatar,
                }
              : null,
            instagram: instagramId
              ? { id: instagramId, needsFetch: true }
              : null,
  ...
  private static async processInstagramSearchResults(
    influencerIds: string[],
  ): Promise<any[]> {
    const startTime = Date.now();
      ...
      influencerIds.map(async (id: string, index: number) => {
        ...
        try {
          ...
          if (igData.youtubeId && igData.youtubeId.trim() !== '') {
            ...
            if (
              potentialYoutubeId !== igData.instagramId &&
              potentialYoutubeId !== id
            ) {
              youtubeId = potentialYoutubeId;
            } else {
              console.warn(
                `⚠️ [INSTAGRAM] youtubeId parece ser el mismo que instagramId: ${potentialYoutubeId}`,
              );
            }
          }
          ...
          if (igData.tiktokId && igData.tiktokId.trim() !== '') {
            ...
            if (
              potentialTiktokId !== igData.instagramId &&
              potentialTiktokId !== id
            ) {
              tiktokId = potentialTiktokId;
            } else {
              console.warn(
                `⚠️ [INSTAGRAM] tiktokId parece ser el mismo que instagramId: ${potentialTiktokId}`,
              );
            }
          }

          const instagramId =
            igData.instagramId && igData.instagramId.trim() !== ''
              ? igData.instagramId
              : null;
          ...
          if (
            ...
            bestAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(
              (igData.instagramName || 'U').charAt(0),
            )}&background=6366f1&color=fff&size=128`;
          }
   ...
   */
  private static async processTikTokSearchResults(
    influencerIds: string[],
  ): Promise<any[]> {
    const startTime = Date.now();
    ...
    const allResults = await Promise.allSettled(
      influencerIds.map(async (id: string, index: number) => {
        ...
        try {
          ...
          const youtubeId =
            ttData.youtubeId && ttData.youtubeId.trim() !== ''
              ? ttData.youtubeId
              : null;
          const instagramId =
            ttData.instagramId && ttData.instagramId.trim() !== ''
              ? ttData.instagramId
              : null;
          const tiktokId =
            ttData.tiktokId && ttData.tiktokId.trim() !== ''
              ? ttData.tiktokId
              : null;
   ...
   */
  private static async processThreadsSearchResults(
    influencerIds: string[],
  ): Promise<any[]> {
    const startTime = Date.now();
    ...
    const allResults = await Promise.allSettled(
      influencerIds.map(async (id: string, index: number) => {
        ...
        try {
          ...
          const threadsId =
            thData.threadsId && thData.threadsId.trim() !== ''
              ? thData.threadsId
              : null;
   ...
   */
  private static async processFacebookSearchResults(
    influencerIds: string[],
  ): Promise<any[]> {
    const startTime = Date.now();
    ...
    const allResults = await Promise.allSettled(
      influencerIds.map(async (id: string, index: number) => {
        ...
        try {
          ...
          const facebookId =
            fbData.facebookId && fbData.facebookId.trim() !== ''
              ? fbData.facebookId
              : null;
          ...
          if (
            ...
            bestAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(
              (fbData.facebookName || 'U').charAt(0),
            )}&background=6366f1&color=fff&size=128`;
          }
   ...
   */
  private static validateCreatorDBResults(
    influencers: any[],
    ...
    influencers.forEach((influencer) => {
      ...
      try {
        ...
        const platformData = this.extractPlatformDataForValidation(
          influencer,
        ...
        // ✅ VALIDAR FOLLOWERS
        if (
          appliedFilters.minFollowers &&
          followers < appliedFilters.minFollowers
        ) {
          isValid = false;
          reasons.push(
            `${followers} followers < ${appliedFilters.minFollowers} (mín)`,
          );
        }

        if (
          appliedFilters.maxFollowers &&
          appliedFilters.maxFollowers < 100000000 &&
          followers > appliedFilters.maxFollowers
        ) {
          isValid = false;
          reasons.push(
            `${followers} followers > ${appliedFilters.maxFollowers} (máx)`,
          );
        }
        ...
        if (engagement !== null && engagement !== undefined) {
          ...
          if (
            appliedFilters.minEngagement &&
            engagementPercent < appliedFilters.minEngagement
          ) {
            isValid = false;
            reasons.push(
              `${engagementPercent.toFixed(2)}% engagement < ${
                appliedFilters.minEngagement
              }% (mín)`,
            );
          }
          ...
          if (
            appliedFilters.maxEngagement &&
            appliedFilters.maxEngagement < 100 &&
            engagementPercent > appliedFilters.maxEngagement
          ) {
            isValid = false;
            reasons.push(
              `${engagementPercent.toFixed(2)}% engagement > ${
        ...
        if (appliedFilters.country && appliedFilters.country !== 'all') {
          const influencerCountry =
            influencer.country ||
            influencer.platformInfo?.youtube?.country ||
            influencer.platformInfo?.instagram?.country ||
            influencer.platformInfo?.tiktok?.country;

          if (
            influencerCountry &&
            influencerCountry !== appliedFilters.country
          ) {
            isValid = false;
            reasons.push(
              `País ${influencerCountry} ≠ ${appliedFilters.country}`,
            );
          }
        }
   ...
   */
  private static extractPlatformDataForValidation(
    influencer: any,
    platform?: string,
  ) {
    try {
      ...
      for (const [platformName, data] of Object.entries(platformInfo)) {
        if (data && typeof data === 'object') {
          const followers =
            (data as any).followers || (data as any).subscribers;
          const engagement =
            (data as any).engageRate || (data as any).engageRate1Y;
  ...
  static async smartSearch(query: string, platform: string) {
    ...
    try {
      ...
      if (platform === 'all') {
        ...
        const instagramResults = await this.searchInstagramSmart(
          query,
          searchType,
        );
        ...
        const crossPlatformResults = await this.extractCrossPlatformIds(
          enrichedInstagramResults,
        );
        ...
        results = instagramIds.map((id: string) => ({
          id,
          platform: 'instagram',
        }));
      ...
      return {
        ...
        priorityPlatform:
          platform === 'all' ? 'instagram' : platform.toLowerCase(),
      };
    } catch (error: any) {
      const totalTime = Date.now() - startTime;
   ...
   */
  private static detectSearchType(
    query: string,
  ): 'username' | 'name' | 'hashtag' | 'keyword' {
   ...
   */
  private static async extractCrossPlatformIds(
    ...
    for (const instagramData of instagramResults) {
      try {
        ...
        if (platformInfo.youtube?.youtubeId) {
          try {
            const youtubeData = await this.getYoutubeBasic(
            ...
            console.warn(
              `⚠️ [CROSS-PLATFORM] Error obteniendo datos de YouTube:`,
              youtubeError,
            );
          }
        }
        ...
        if (platformInfo.tiktok?.tiktokId) {
          try {
            const tiktokData = await this.getTikTokBasic(
            ...
            console.warn(
              `⚠️ [CROSS-PLATFORM] Error obteniendo datos de TikTok:`,
              tiktokError,
            );
          }
        }
        ...
        if (platformInfo.threads?.threadsId) {
          try {
            const threadsData = await this.getThreadsBasic(
              platformInfo.threads.threadsId,
            );
            ...
            console.warn(
              `⚠️ [CROSS-PLATFORM] Error obteniendo datos de Threads:`,
              threadsError,
            );
          }
        }
        ...
        if (platformInfo.facebook?.facebookId) {
          try {
            const facebookData = await this.getFacebookBasic(
              platformInfo.facebook.facebookId,
            );
            ...
            console.warn(
              `⚠️ [CROSS-PLATFORM] Error obteniendo datos de Facebook:`,
              facebookError,
            );
          }
        }
        ...
        console.warn(
          `⚠️ [CROSS-PLATFORM] Error extrayendo IDs cruzados:`,
          error,
        );
      }
    }
  ...
  private static generateSearchVariants(query: string): string[] {
    ...
    const filteredVariants = Array.from(variants).filter(
      (variant) => variant.length >= 2 && variant.length <= 20,
    );
   ...
   */
  private static async searchYoutubeSmart(
    query: string,
    searchType: string,
  ): Promise<any[]> {
    ...
    try {
      ...
      const response = await creatorDBClient.post(
      ...
      try {
        const fallbackFilters = [
          { filterKey: 'youtubeName', op: '=', value: cleanQuery },
        ];
        const fallbackBody = {
          maxResults: 20,
          offset: 0,
        ...
        const fallbackResponse = await creatorDBClient.post(
          '/youtubeAdvancedSearch',
          fallbackBody,
        );
   ...
   */
  private static async searchInstagramSmart(
    query: string,
    searchType: string,
  ): Promise<any[]> {
    const startTime = Date.now();
    ...
    try {
      ...
      const directPromises = searchVariants.map(
        async (variant: string, index: number) => {
      ...
      const hasMinFollowers = body.filters.some(
        (f: any) => f.filterKey === 'followers' && f.op === '>',
      );
      if (!hasMinFollowers) {
   ...
   */
  private static async searchTikTokSmart(
    query: string,
    searchType: string,
  ): Promise<any[]> {
    const startTime = Date.now();
   ...
   */
  private static async enrichSearchResults(
    results: any[],
    platform: string,
  ): Promise<any[]> {
    const startTime = Date.now();
        ...
        batch.map(async (result: any) => {
          try {
            ...
            if (sourcePlatform === 'instagram' && platform === 'all') {
              ...
              try {
                const instagramData = await CreatorDBService.getInstagramBasic(
                  id,
                );
                if (instagramData?.basicInstagram) {
                  ...
                  if (linkedIds.youtubeId) {
                    linkedPlatformPromises.push(
                      CreatorDBService.getYoutubeBasic(linkedIds.youtubeId)
                        .then((data) => ({
                          platform: 'youtube',
                          data: data?.basicYoutube,
                        }))
                        .catch((err) => {
                          console.warn(
                            `⚠️ Error obteniendo YouTube ${linkedIds.youtubeId}:`,
                            err.message,
                          );
                  ...
                  if (linkedIds.tiktokId) {
                    linkedPlatformPromises.push(
                      CreatorDBService.getTikTokBasic(linkedIds.tiktokId)
                        .then((data) => ({
                          platform: 'tiktok',
                          data: data?.basicTikTok,
                        }))
                        .catch((err) => {
                          console.warn(
                            `⚠️ Error obteniendo TikTok ${linkedIds.tiktokId}:`,
                            err.message,
                          );
                  ...
                  if (linkedIds.facebookId) {
                    linkedPlatformPromises.push(
                      CreatorDBService.getFacebookBasic(linkedIds.facebookId)
                        .then((data) => ({
                          platform: 'facebook',
                          data: data?.basicFacebook,
                        }))
                        .catch((err) => {
                          console.warn(
                            `⚠️ Error obteniendo Facebook ${linkedIds.facebookId}:`,
                            err.message,
                          );
                  ...
                  if (linkedIds.threadsId) {
                    linkedPlatformPromises.push(
                      CreatorDBService.getThreadsBasic(linkedIds.threadsId)
                        .then((data) => ({
                          platform: 'threads',
                          data: data?.basicThreads,
                        }))
                        .catch((err) => {
                          console.warn(
                            `⚠️ Error obteniendo Threads ${linkedIds.threadsId}:`,
                            err.message,
                          );
                  ...
                  if (linkedPlatformPromises.length > 0) {
                    ...
                    const obtainedPlatforms = Object.keys(enrichedData).filter(
                      (key) =>
                        key !== 'id' &&
                        key !== 'platform' &&
                  ...
                  const processedInfluencer =
                    this.convertToInfluencerFormat(enrichedData);
                    ...
                    if (linkedPromises.length > 0) {
                      ...
                      linkedResults.forEach((result) => {
                        if (
                          ...
                          enrichedData[result.value.platform] =
                            result.value.data;
                        }
                      });
                    }
  ...
  private static deduplicateInfluencers(influencers: any[]): any[] {
    ...
    for (const influencer of influencers) {
      ...
      const identifier =
        creatorId || name || username || `unknown_${Date.now()}`;
      ...
      if (!seen.has(primaryKey)) {
        ...
        const existingPlatforms = Object.values(
          existing.platformInfo || {},
        ).filter((p) => p !== null).length;
        const currentPlatforms = Object.values(
        ...
        const shouldReplace =
          ...
          ((influencer.followersCount || 0) ===
            ...
            currentPlatforms > existingPlatforms) ||
          ((influencer.followersCount || 0) ===
            (existing.followersCount || 0) &&
            currentPlatforms === existingPlatforms &&
            (influencer.averageEngagementRate || 0) >
              (existing.averageEngagementRate || 0));
   ...
   */
  private static buildInstagramSearchBody(
    ...
    if (filters.minEngagement) {
      const minEngagementValue =
        parseFloat(filters.minEngagement.toString()) / 100; // Convertir % a decimal
    ...
    if (filters.maxEngagement) {
      const maxEngagementValue =
        parseFloat(filters.maxEngagement.toString()) / 100; // Convertir % a decimal
    ...
    if (
      filters.nicheIds &&
      Array.isArray(filters.nicheIds) &&
    ...
    if (filters.minGRateFollowers || filters.maxGRateFollowers) {
      const minValue = filters.minGRateFollowers
        ? parseFloat(filters.minGRateFollowers.toString())
        : 0;
      const maxValue = filters.maxGRateFollowers
        ? parseFloat(filters.maxGRateFollowers.toString())
        : 1;

      if (filters.minGRateFollowers) {
        body.filters.push({
          filterKey: 'gRateFollowers',
          op: '>',
          value: minValue,
        });
      }
   ...
   */
  private static buildYoutubeSearchBody(
    filters: Record<string, any>,
  ): Record<string, any> {
    const {
      page = 1,
    ...
    if (minEngagement) {
      ...
      apiFilters.push({
        filterKey: 'engageRate1Y',
        op: '>',
        value: minEngagementValue / 100,
      });
    }
    ...
    if (maxEngagement) {
      ...
      apiFilters.push({
        filterKey: 'engageRate1Y',
        op: '<',
        value: maxEngagementValue / 100,
      });
    }

    if (username) {
      apiFilters.push({
        filterKey: 'youtube_handle',
        op: '=',
        value: username,
      });
    }
   ...
   */
  private static buildTikTokSearchBody(
    filters: Record<string, any>,
  ): Record<string, any> {
    // 🎯 NUEVO: Siempre pedir 100 resultados para optimizar caché
    const CACHE_BATCH_SIZE = 50;
   ...
   */
  private static buildThreadsSearchBody(
    filters: Record<string, any>,
  ): Record<string, any> {
    // 🎯 NUEVO: Siempre pedir 100 resultados para optimizar caché
    const CACHE_BATCH_SIZE = 50;
    ...
    if (filters.minEngagement) {
      body.filters.push({
        filterKey: 'gRateThreadsTabAvgLikes',
        op: '>',
        value: filters.minEngagement / 100,
      });
    }
    if (filters.maxEngagement) {
      body.filters.push({
        filterKey: 'gRateThreadsTabAvgLikes',
        op: '<',
        value: filters.maxEngagement / 100,
      });
    }
   ...
   */
  private static buildFacebookSearchBody(
    filters: Record<string, any>,
  ): Record<string, any> {
    // 🎯 NUEVO: Siempre pedir 50 resultados para optimizar caché
    const CACHE_BATCH_SIZE = 50;
  ...
  static async searchInstagramInfluencer(username: string) {
    const response = await creatorDBClient.get('/instagram/search', {
      params: { username },
    });
    return response.data;
  }
  ...
  static async searchTikTokInfluencer(username: string) {
    const response = await creatorDBClient.get('/tiktok/search', {
      params: { username },
    });
    return response.data;
  }
  ...
  static async searchYouTubeInfluencer(username: string) {
    const response = await creatorDBClient.get('/youtube/search', {
      params: { username },
    });
    return response.data;
  }
  ...
  private static convertToInfluencerFormat(enrichedData: any) {
    try {
      ...
      const rawName =
        primaryData.instagramName ||
        primaryData.youtubeName ||
        primaryData.tiktokName ||
        primaryData.facebookName ||
        primaryData.threadsName ||
        'Sin nombre';

      // Si el nombre es "Sin nombre" o contiene números, usar el ID
      const displayName =
        rawName === 'Sin nombre' || /\d/.test(rawName) ? id : rawName;
   ...
   */
  private static validateAPIFilters(
    filters: Record<string, any>,
    platform: string,
    ...
    switch (platform.toLowerCase()) {
      case 'instagram':
        ...
        if (
          filters.minEngagement &&
          (filters.minEngagement < 0 || filters.minEngagement > 100)
        ) {
          errors.push('Instagram: minEngagement debe estar entre 0 y 100');
        }
        if (
          filters.maxEngagement &&
          (filters.maxEngagement < 0 || filters.maxEngagement > 100)
        ) {
      ...
      case 'youtube':
        ...
        if (filters.maxFollowers && filters.maxFollowers > 1000000000) {
          errors.push(
            'YouTube: maxFollowers (subscribers) debe ser <= 1,000,000,000',
          );
        }
        break;
    ...
    if (filters.country && filters.country.length !== 3) {
      errors.push(
        'country debe ser un código ISO de 3 letras (ej: ARG, USA, ESP)',
      );
    }
   ...
   */
  private static estimateCountForPrefetch(
    currentCount: number,
    size: number,
    page: number,
  ): number {
   ...
   */
  static async getInfluencersByIds(
    platformIds: { platform: string; ids: string[] }[],
  ) {
