...
export class CampaignService {
  ...
  private async getMyCampaignsWithMetricsOptimized(organizationIds: string[]): Promise<any[]> {
    ...
    try {
      ...
      if (campaignsError) {
        console.error('❌ [SERVICE] Error fetching base campaigns:', campaignsError);
      ...
      const [influencersData, postsData, metricsData] = await Promise.all([
        ...
        supabase
          .from('post_metrics')
          .select('post_url, engagement_rate, likes_count, comments_count, views_count')
          .not('post_url', 'is', null)
      ]);

      // 🚀 STEP 3: Procesar errores de queries paralelas
      if (influencersData.error) {
        console.warn('⚠️ [SERVICE] Influencers query failed:', influencersData.error);
      }
      if (postsData.error) {
        ...
        if (campaignPosts.length > 0) {
          ...
          campaignPosts.forEach(post => {
            ...
            if (metric.engagement_rate && metric.engagement_rate > 0) {
              const percentage = metric.engagement_rate < 1 
                ? metric.engagement_rate * 100 
                : metric.engagement_rate;
              validEngagementRates.push(Math.min(percentage, 100)); // Cap at 100%
            } else if (metric.views_count > 0 && (metric.likes_count > 0 || metric.comments_count > 0)) {
              // Calcular engagement manualmente
              const calculatedRate = ((metric.likes_count + metric.comments_count) / metric.views_count) * 100;
          ...
          if (validEngagementRates.length > 0) {
            avgEngagement = validEngagementRates.reduce((sum, rate) => sum + rate, 0) / validEngagementRates.length;
          }
        }
  ...
  private async getMyCampaignsWithMetricsOptimizedForIds(campaignIds: string[]): Promise<any[]> {
    try {
      ...
      if (campaignsError) {
        console.error('❌ [SERVICE] Error fetching base campaigns by IDs:', campaignsError);
        throw campaignsError;
      }
      ...
      const influencersCount = influencersData.data?.reduce((acc: any, item: any) => {
        acc[item.campaign_id] = (acc[item.campaign_id] || 0) + 1;
        return acc;
      }, {}) || {};

      const postsCount = postsData.data?.reduce((acc: any, item: any) => {
        acc[item.campaign_id] = (acc[item.campaign_id] || 0) + 1;
        return acc;
      }, {}) || {};
      ...
      console.error('❌ [SERVICE] Error in getMyCampaignsWithMetricsOptimizedForIds:', error);
      throw error;
    }
  }
  ...
  async addInfluencer(data: CampaignInfluencerCreateDTO): Promise<CampaignInfluencer> {
  ...
  async updateInfluencerStatus(campaignId: string, influencerId: string, data: CampaignInfluencerUpdateDTO): Promise<CampaignInfluencer> {
    const updateData = {
      ...data,
      updated_at: new Date().toISOString()
    };
  ...
  async removeInfluencer(campaignId: string, influencerId: string): Promise<void> {
    ...
    try {
      ...
      console.error('❌ [CampaignService] Error en proceso de eliminación:', error);
      throw error;
    }
  }

  async createContent(data: CampaignContentCreateDTO): Promise<CampaignContent> {
    const contentData = {
      ...data,
      status: 'draft',
      created_at: new Date().toISOString(),
  ...
  async updateContent(id: string, data: CampaignContentUpdateDTO): Promise<CampaignContent> {
    const updateData = {
      ...data,
      updated_at: new Date().toISOString()
    };
  ...
  async getInfluencerContent(campaignId: string, influencerId: string): Promise<CampaignContent[]> {
    const { data, error } = await supabase
      .from('campaign_content')
      .select()
      .eq('campaign_id', campaignId)
  ...
  async updateCampaignMetrics(id: string, metrics: Campaign['metrics']): Promise<Campaign> {
    const { data: campaign, error } = await supabase
      .from('campaigns')
      .update({
        metrics,
  ...
  async assignUsersToCampaign(campaignId: string, userIds: string[]): Promise<void> {
    ...
    if (campaignError || !campaign) {
      console.error('❌ [CAMPAIGN_SERVICE] Error al obtener campaña:', campaignError);
    ...
    if (invalidUserIds.length > 0) {
      throw new Error(`Los siguientes usuarios no pertenecen a la organización: ${invalidUserIds.join(', ')}`);
    }
