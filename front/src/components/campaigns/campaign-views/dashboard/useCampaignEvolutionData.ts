...
// ✅ NUEVA FUNCIÓN: Aplicar acumulación progresiva para evitar pendientes negativas
...
// Función para obtener todas las métricas de evolución desde el backend
const getAllMetricsEvolution = async (campaignId: string, posts: any[]): Promise<MetricEvolution[]> => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to fetch all metrics: ${response.status} - ${errorText}`);
    }
    ...
    const evolutionArray = applyProgressiveAccumulation(consolidatedMetrics, posts);
    ...
    return getRealEvolutionData(posts);
  }
};

// Función mejorada para obtener datos de evolución desde los posts locales
const getRealEvolutionData = (posts: any[]): MetricEvolution[] => {
  ...
  return applyProgressiveAccumulation(consolidatedMetrics, posts);
};

// Función alternativa robusta para generar datos de evolución
const generateRobustEvolutionData = (posts: any[]): MetricEvolution[] => {
  if (!posts || posts.length === 0) {
    return [];
  }
  ...
  // Estrategia 1: Usar post_metrics.created_at (más preciso)
  const metricsByDay: Record<string, { reach: number; engagement: number; count: number }> = {};
  ...
  validPosts.forEach(post => {
    ...
    // Priorizar post_metrics.created_at
    if (post.post_metrics?.created_at) {
      dateToUse = post.post_metrics.created_at.split('T')[0];
    } else if (post.created_at) {
      dateToUse = post.created_at.split('T')[0];
  ...
  // Si no hay datos suficientes, intentar estrategia alternativa
  if (Object.keys(consolidatedMetrics).length === 0) {
    // Estrategia 2: Usar fechas de posts y distribuir métricas
    const postsByDay: Record<string, any[]> = {};
    
    validPosts.forEach(post => {
      const postDate = post.created_at?.split('T')[0] || post.updated_at?.split('T')[0];
      if (postDate) {
        if (!postsByDay[postDate]) {
          postsByDay[postDate] = [];
        }
        postsByDay[postDate].push(post);
      }
    });

    // Consolidar TODOS los días impares en días pares para la estrategia alternativa
    const consolidatedPostsByDay: Record<string, any[]> = {};
    const sortedPostDates = Object.keys(postsByDay).sort();
    
    for (let i = 0; i < sortedPostDates.length; i++) {
      const currentDate = sortedPostDates[i];
      const [year, month, day] = currentDate.split('-').map(Number);
      const currentDayNumber = day;
      ...
      // Consolidar posts en la fecha objetivo
      if (!consolidatedPostsByDay[targetDate]) {
        consolidatedPostsByDay[targetDate] = [];
      }
      consolidatedPostsByDay[targetDate].push(...postsByDay[currentDate]);
    }

    // Generar datos de evolución basados en posts consolidados por día
    return Object.keys(consolidatedPostsByDay)
      .sort()
      .map(dateKey => {
        const dayPosts = consolidatedPostsByDay[dateKey];
        const totalReach = dayPosts.reduce((sum, post) => {
          const metrics = extractMetricsFromRawResponse(post);
          return sum + metrics.reach;
        }, 0);
        ...
        const totalEngagement = dayPosts.reduce((sum, post) => {
          const metrics = extractMetricsFromRawResponse(post);
          return sum + metrics.engagement;
        }, 0);

        return {
          date: dateKey,
          reach: totalReach,
          engagement: dayPosts.length > 0 ? totalEngagement / dayPosts.length : 0
        };
      });
  }
...
// Función para generar datos de evolución con fechas simuladas cuando no hay suficientes datos
const generateSimulatedEvolutionData = (posts: any[]): MetricEvolution[] => {
  if (!posts || posts.length === 0) {
    return [];
  }
...
export const useCampaignEvolutionData = (campaignId: string) => {
  const [evolutionData, setEvolutionData] = useState<MetricEvolution[]>([]);
  const [evolutionLoading, setEvolutionLoading] = useState(true);
  const [hasAttemptedLoad, setHasAttemptedLoad] = useState(false);
  
  // Obtener posts de la campaña para verificar si tiene contenido
  ...
  useEffect(() => {
    const fetchEvolutionData = async () => {
      try {
        ...
        let data: MetricEvolution[] = [];
        try {
          data = await getAllMetricsEvolution(campaignId, campaignPosts);
        } catch (backendError) {
        ...
        if (!data || data.length === 0) {
          data = generateRobustEvolutionData(campaignPosts);
        }
        
        // Si aún no hay datos suficientes, usar simulación
        if (!data || data.length <= 1) {
          data = generateSimulatedEvolutionData(campaignPosts);
        }
        ...
        let fallbackData = generateRobustEvolutionData(campaignPosts);
        
        // Si no hay datos suficientes, usar simulación
        if (!fallbackData || fallbackData.length <= 1) {
          fallbackData = generateSimulatedEvolutionData(campaignPosts);
        }
    ...
    // Solo ejecutar si tenemos campaña y posts
    if (campaignId && campaignPosts.length > 0) {
      setHasAttemptedLoad(true);
      fetchEvolutionData();
    } else if (campaignId && !postsLoading) {
      // Si no hay posts pero ya terminó de cargar, marcar como no cargando
      setHasAttemptedLoad(true);
      setEvolutionLoading(false);
    }
  }, [campaignId, campaignPosts, postsLoading]);
