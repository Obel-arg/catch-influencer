...
export const AddPostModal: React.FC<AddPostModalProps> = ({
  ...
  // Estados para Instagram Stories
  ...
  useEffect(() => {
    if (formData.platform !== 'instagram') {
      setPostType('post');
      setFormData((prev) => ({ ...prev, content_type: 'post' }));
    }
  }, [formData.platform]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation for Instagram stories
    if (formData.platform === 'instagram' && postType === 'story') {
      if (!formData.image_url) {
        alert('Por favor sube un screenshot de la historia');
        return;
      }
    } else {
    ...
    try {
      ...
      // For stories, use placeholder URL
      if (formData.platform === 'instagram' && postType === 'story') {
        postDataToSubmit.post_url = `https://www.instagram.com/stories/${
          influencerName || 'user'
        }/${Date.now()}`;
        postDataToSubmit.content_type = 'story';
        // Metrics will be added later via "Ver análisis" button
      }
    ...
    if (isInstagramUrl(url) && !formData.image_url) {
      ...
      try {
        ...
        if (thumbnail) {
          ...
          if (
            thumbnail.includes('instagram.com') ||
            thumbnail.includes('cdninstagram.com') ||
            thumbnail.includes('fbcdn.net')
          ) {
            finalUrl = `https://images.weserv.nl/?url=${encodeURIComponent(
              thumbnail,
            )}&w=800&h=800&fit=cover&output=webp`;
          } else {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      const fetchResponse = await fetch(
        `${apiBaseUrl}/influencer-posts/upload-image`,
        {
          method: 'POST',
          headers: {
      ...
      const responseData = (await fetchResponse.json()) as {
        success: boolean;
        data?: { imageUrl: string; uploadedAt: string };
        message?: string;
      };
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-white">
          ...
          {isInitialLoading ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* URL del Post - Hidden for Instagram Stories */}
              {!(formData.platform === 'instagram' && postType === 'story') && (
                <div className="space-y-2">
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-gray-500">
                              {capitalizeFirst(schedule.platform)} •{' '}
                              {capitalizeFirst(schedule.content_type)}
                            </div>
                          </div>
                        </div>
                      ))
              ...
              {/* Post Type Selector - Only for Instagram */}
              {formData.platform === 'instagram' && (
                <div className="space-y-2">
                  ...
                  <Select
                    value={postType}
                    onValueChange={(value: 'post' | 'story') => {
                      setPostType(value);
              ...
              {/* Screenshot Upload for Instagram Stories or URL Input for other posts */}
              {formData.platform === 'instagram' && postType === 'story' ? (
                <div className="space-y-2">
                  <Label className="text-sm font-medium">
                    Screenshot de la Historia{' '}
                    ...
                    <div className="space-y-2">
                      <div className="relative inline-block w-full">
                        <SmartImage
                          ...
                          className="w-full max-w-md h-48 object-cover border rounded-md"
                          fallback={
                            <div className="w-full max-w-md h-48 border rounded-md bg-gray-50 flex items-center justify-center text-sm text-gray-500">
                              Error al cargar
                            </div>
                          }
                        />
                        ...
                        <button
                          ...
                          className="absolute top-2 right-2 bg-white/90 hover:bg-white border border-gray-300 rounded-md px-3 py-2 shadow-sm transition-all duration-200 hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                        >
                          {isUploadingImage ? (
