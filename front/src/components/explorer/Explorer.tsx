...
export default function Explorer() {
  ...
  const { trackRequest, completeRequest, createRequestContext } = useRequestMonitoring('Explorer');
  ...
  const getSmartAvatar = useCallback((influencer: any): string => {
    ...
    const processedUrl = getSafeAvatarUrlForModal(originalAvatar, influencerName);
  ...
  const SmartAvatar = ({ influencer }: { influencer: any }) => {
    ...
    useEffect(() => {
      try {
        ...
        const fallbackUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName?.charAt(0) || 'U')}&background=6366f1&color=fff&size=128`;
        setAvatarUrl(fallbackUrl);
        setLoading(false);
      }
    }, [influencer.creatorId, influencer.avatar, influencer.image, displayName]);
  ...
  const adaptedInfluencers = useMemo(() => {
    ...
    const adapted = influencers.map((inf, index) => {
      ...
      if (platformInfo.youtube && platformInfo.youtube !== null) {
        socialPlatforms.push({
          platform: 'YouTube',
          username: platformInfo.youtube.youtubeName || platformInfo.youtube.displayName || '',
          followers: platformInfo.youtube.subscribers || 0,
          engagement: platformInfo.youtube.engageRate1Y || platformInfo.youtube.engageRate || 0
        });
      }
      
      if (platformInfo.instagram && platformInfo.instagram !== null) {
        const instagramData = platformInfo.instagram.basicInstagram || platformInfo.instagram;
        socialPlatforms.push({
          platform: 'Instagram',
      ...
      if (platformInfo.facebook && platformInfo.facebook !== null) {
        const facebookData = platformInfo.facebook.basicFacebook || platformInfo.facebook;
        socialPlatforms.push({
          platform: 'Facebook',
      ...
      if (platformInfo.threads && platformInfo.threads !== null) {
        const threadsData = platformInfo.threads.basicThreads || platformInfo.threads;
        socialPlatforms.push({
          platform: 'Threads',
          username: threadsData.threadsId || threadsData.username || '',
          followers: threadsData.followers || 0,
          engagement: threadsData.gRateThreadsTabAvgLikes || threadsData.engageRate || 0
        });
      }
      
      // 🎯 NUEVO ORDEN: Instagram primero cuando se busca en "all" platforms
      const platformOrder = platform === "all" 
      ...
      const mainPlatformData = (() => {
        ...
        if (platform !== "all" && socialPlatforms.length > 0) {
          const specificPlatform = socialPlatforms.find(p => 
        ...
        if (platform === "all" && socialPlatforms.length > 0) {
          const instagramPlatform = socialPlatforms.find(p => p.platform === 'Instagram');
        ...
        const fallbackPlatform = socialPlatforms.length > 0 
      ...
      const totalFollowers = inf.followerBreakdown?.total || inf.followersCount || mainPlatformData.followers;
      const averageEngagement = inf.averageEngagementRate || mainPlatformData.engagement;
      ...
      const adaptedInfluencer = {
        ...
        socialPlatforms,
        mainSocialPlatform: inf.mainSocialPlatform || mainPlatformData.platform, // Usar del backend si está disponible
    ...
    // 🎯 LOG ESTADÍSTICAS
    const multiPlatformInfluencers = adapted.filter(inf => inf.socialPlatforms.length > 1);
    const instagramPrimaryCount = adapted.filter(inf => inf.mainSocialPlatform === 'Instagram').length;
  ...
  const detectAvailablePlatforms = (influencer: any) => {
    ...
    // Detectar YouTube - CORREGIDO: Verificar si tiene ID, no followers
    if (platformInfo.youtube) {
      const ytData = platformInfo.youtube;
      const hasYouTubeId = ytData.youtubeName || ytData.displayId || ytData.channelId || ytData.id || ytData.youtubeId;
      if (hasYouTubeId) {
    ...
    // Detectar Instagram - CORREGIDO: Verificar si tiene ID, no followers
    if (platformInfo.instagram) {
      let instagramData = platformInfo.instagram;
      
      // Si viene envuelto en basicInstagram, extraer
    ...
    // Detectar TikTok - CORREGIDO: Verificar si tiene ID, no followers
    if (platformInfo.tiktok) {
      let tiktokData = platformInfo.tiktok;
      
      // Si viene envuelto en basicTikTok, extraer
      ...
      if (hasTikTokId) {
        const followers = tiktokData.followers || 0;
        platforms.push({
          name: 'TikTok',
          followers: followers
        });
    ...
    // Detectar Facebook - CORREGIDO: Verificar si tiene ID, no followers
    if (platformInfo.facebook) {
      ...
      if (hasFacebookId) {
        const followers = facebookData.followers || 0;
        platforms.push({
          name: 'Facebook',
          followers: followers
        });
    ...
    // Detectar Threads - CORREGIDO: Verificar si tiene ID, no followers
    if (platformInfo.threads) {
      ...
      if (hasThreadsId) {
        const followers = threadsData.followers || 0;
        platforms.push({
          name: 'Threads',
          followers: followers
        });
    ...
    if (platforms.length === 0) {
      ...
      // Si no, detectar por avatar URL como último recurso
        ...
        if (avatar.includes('googleusercontent.com') || avatar.includes('ytimg.com') || avatar.includes('ggpht.com')) {
          ...
          platforms.push({ name: 'Instagram', followers: 0 });
        } else if (avatar.includes('tiktokcdn.com') || avatar.includes('muscdn.com')) {
          platforms.push({ name: 'TikTok', followers: 0 });
        }
  ...
  const adaptFullInfluencerForPanel = (inf: any) => {
    ...
    if (platformInfo.youtube && platformInfo.youtube !== null) {
      let username = platformInfo.youtube.displayId || platformInfo.youtube.youtubeName || '';
    ...
    const mainPlatformInfo = [...socialPlatforms].sort((a, b) => b.followers - a.followers)[0] || null;
  ...
  const openInfluencerPanel = async (influencer: any) => {
    ...
    try {
      ...
      if (fullInfluencerCache[cacheKey]) {
        ...
        setSelectedInfluencer(adaptFullInfluencerForPanel(fullInfluencerCache[cacheKey]));
        setLoadingPanel(false);
      ...
      if (allPlatformIds.youtubeId) {
        try {
          ...
          const { InfluencerExtendedService } = await import('@/lib/services/influencer/influencer-extended.service');
          ...
          const extendedData = await InfluencerExtendedService.readExistingExtendedData(allPlatformIds.youtubeId);
          
          if (extendedData && extendedData.data && Object.keys(extendedData.data).length > 0) {
            // ✅ DATOS ENCONTRADOS EN BD LOCAL
            const adaptedData = adaptExtendedDataForPanel(extendedData.data, influencer);
            setFullInfluencerCache((prev) => ({ ...prev, [cacheKey]: adaptedData }));
          ...
          completeRequest(requestId, 204, { message: 'No data in local DB' }, true);
        } catch (dbError) {
      ...
      if (data && Object.keys(data).length > 0) {
        ...
        completeRequest(requestId, 204, { message: 'No data from external API' }, false);
      }
  ...
  const adaptExtendedDataForPanel = (extendedData: any, basicInfluencer: any) => {
  ...
  const locations: string[] = Array.from(new Set(influencers.map((inf) => inf.location)));
  const categories: string[] = Array.from(new Set(influencers.map((inf) => inf.categories?.[0] || "")));
  ...
  async function saveIfNotExists(fullData: any) {
    try {
      const existing = await influencerService.getInfluencerById(fullData.creatorId);
  ...
  const getLanguageName = (languageCode: string | undefined): string => {
    ...
    return languageMap[normalizedCode] || languageMap[languageCode] || languageCode.toUpperCase();
  };

  const adaptBasicPlatformDataForPanel = (data: any, originalInfluencer: any) => {
    ...
    if (platformInfo.youtube && platformInfo.youtube !== null) {
      const youtubeData = platformInfo.youtube.basicYoutube || platformInfo.youtube;
      let username = youtubeData.displayId || youtubeData.youtubeName || youtubeData.channelTitle || '';
      if (username.startsWith('@')) {
        username = username.substring(1);
      }
      socialPlatforms.push({
  ...
  const extractAllPlatformIds = (influencer: any) => {
    ...
    if (platformInfo.youtube) {
      ...
      // ID de YouTube
      platformIds.youtubeId = youtubeData.displayId || youtubeData.id || youtubeData.youtubeId;
      
      // 🎯 IDs ADICIONALES que pueden venir en la respuesta de YouTube
      platformIds.instagramId = youtubeData.instagramId || platformIds.instagramId;
    ...
    if (platformInfo.instagram) {
      ...
      platformIds.instagramId = basicInstagram.instagramId || basicInstagram.id || basicInstagram.username || platformIds.instagramId;
      
      // 🎯 IDs ADICIONALES que pueden venir en la respuesta de Instagram
      platformIds.youtubeId = basicInstagram.youtubeId || basicInstagram.youtubeChannelId || platformIds.youtubeId;
      platformIds.tiktokId = basicInstagram.tiktokId || basicInstagram.tiktokUsername || platformIds.tiktokId;
      platformIds.facebookId = basicInstagram.facebookId || basicInstagram.facebookPageId || platformIds.facebookId;
      platformIds.threadsId = basicInstagram.threadsId || basicInstagram.threadsUsername || platformIds.threadsId;
    }
    ...
    if (platformInfo.tiktok) {
      ...
      platformIds.tiktokId = basicTikTok.tiktokId || basicTikTok.id || basicTikTok.username || platformIds.tiktokId;
      
      // 🎯 IDs ADICIONALES que pueden venir en la respuesta de TikTok
      platformIds.youtubeId = basicTikTok.youtubeId || basicTikTok.youtubeChannelId || platformIds.youtubeId;
      platformIds.instagramId = basicTikTok.instagramId || basicTikTok.instagramUsername || platformIds.instagramId;
      platformIds.facebookId = basicTikTok.facebookId || basicTikTok.facebookPageId || platformIds.facebookId;
      platformIds.threadsId = basicTikTok.threadsId || basicTikTok.threadsUsername || platformIds.threadsId;
    }
    ...
    if (platformInfo.facebook) {
      ...
      platformIds.facebookId = basicFacebook.facebookId || basicFacebook.id || basicFacebook.username || platformIds.facebookId;
      
      // 🎯 IDs ADICIONALES que pueden venir en la respuesta de Facebook
      platformIds.youtubeId = basicFacebook.youtubeId || basicFacebook.youtubeChannelId || platformIds.youtubeId;
      platformIds.instagramId = basicFacebook.instagramId || basicFacebook.instagramUsername || platformIds.instagramId;
      platformIds.tiktokId = basicFacebook.tiktokId || basicFacebook.tiktokUsername || platformIds.tiktokId;
      platformIds.threadsId = basicFacebook.threadsId || basicFacebook.threadsUsername || platformIds.threadsId;
    }
    ...
    if (platformInfo.threads) {
      ...
      platformIds.threadsId = basicThreads.threadsId || basicThreads.id || basicThreads.username || platformIds.threadsId;
      
      // 🎯 IDs ADICIONALES que pueden venir en la respuesta de Threads
      platformIds.youtubeId = basicThreads.youtubeId || basicThreads.youtubeChannelId || platformIds.youtubeId;
      platformIds.instagramId = basicThreads.instagramId || basicThreads.instagramUsername || platformIds.instagramId;
      platformIds.tiktokId = basicThreads.tiktokId || basicThreads.tiktokUsername || platformIds.tiktokId;
    ...
    <div className="flex flex-col lg:flex-row gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden" style={{ scrollBehavior: 'auto' }}>
          ...
          {/* Tabla de resultados */}
          <div className="overflow-x-auto bg-white relative min-h-[550px]">
                ...
                <div className="flex-1 min-h-[480px]">
                  <table className="w-full">
                    <tbody className="divide-y divide-gray-200">
                      <SkeletonInfluencerTable selectMode={selectMode} rows={6} />
                    </tbody>
                  </table>
                </div>
              ...
              <tbody className="divide-y divide-gray-200">
                ...
                {(() => {
                        ...
                        <td colSpan={selectMode ? 8 : 7} className="p-0">
                          <div className="flex items-center justify-center min-h-[400px]">
                            <div className="text-center px-4">
                              ...
                              <h3 className="text-xl font-semibold text-gray-900 mb-2">Búsqueda de Influencers</h3>
                              <p className="text-gray-500 max-w-md mx-auto">
                                Utiliza los filtros de la izquierda y haz clic en "Buscar" para encontrar influencers que se adapten a tu campaña.
                              </p>
                            </div>
                          </div>
                        </td>
                  ...
                  return (
                    ...
                    const uniqueKey = `${influencer.id}_${index}_${influencer.name?.replace(/\s+/g, '').toLowerCase()}`;
                    
                    // Verificar selección
                    const isSelected = selectedInfluencers.includes(influencer.creatorId);
                    ...
                    const selectedClasses = (selectMode && isSelected) ? "bg-blue-50 border-l-4 border-blue-500" : "";
                    const finalClassName = `${baseClasses} ${cursorClass} ${selectedClasses}`.trim();
                    
                    return (
                      <tr 
                        key={uniqueKey} 
                        className={finalClassName}
                        onClick={(e) => handleRowClick(influencer.creatorId, e)}
                      >
                        {selectMode && (
                          <td className="py-4 px-3 text-center">
                            <label className="relative flex items-center justify-center cursor-pointer">
                              <input
                                ...
                                className="sr-only"
                                checked={isSelected}
                                onChange={() => handleSelectInfluencer(influencer.creatorId)}
                              />
              ...
              {!loadingInfluencers && hasEverSearched && limitedInfluencers.length === 0 && (
                <div className="p-4 sm:p-8 text-center">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 mb-1">
                    {page > 1 ? 'No hay más resultados' : 'No se encontraron influencers'}
                  </h3>
      ...
      <ExplorerAssignModal
        ...
        influencersData={adaptedInfluencers.filter(inf => selectedInfluencers.includes(inf.creatorId))}
        campaigns={campaigns}
