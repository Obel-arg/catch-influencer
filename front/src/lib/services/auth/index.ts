...
export class AuthService {
  ...
  public async login(credentials: LoginCredentials): Promise<AuthResponse> {
    const response = await httpAuthClient.post<AuthResponse>(
      `${this.baseUrl}/login`,
      credentials,
      {
  ...
  public async refreshToken(
    ...
    const response = await httpAuthClient.post<RefreshTokenResponse>(
  ...
  public async getGoogleAuthUrl(): Promise<string> {
    const response = await httpAuthClient.get<{ authUrl: string }>(
  ...
  public async loginWithGoogle(): Promise<void> {
    try {
      // Usar getApiBaseUrl() que maneja NEXT_PUBLIC_API_URL automáticamente
  ...
  public async handleGoogleCallback(
    ...
    try {
      ...
      try {
        const userResponse = await httpAuthClient.get<any>(
          `${this.baseUrl}/me`,
          {
            headers: new AxiosHeaders({
        ...
        console.warn(
          "Error calling /me endpoint, decoding token instead:",
          meError
        );
